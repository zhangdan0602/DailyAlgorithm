//
// Created by ZD-Mac on 2021-01-30.
//
...
public:
    ...
    bool dfs(vector<vector<int>> grid, int x, int y, vector<vector<int>> &visited, int threshold) {
        visited[x][y] = 1;
        for (const auto[dr, dc]: directions) {
            int newX = x + dr;
            int newY = y + dc;
            if (newX >= 0 && newX < n && newY >= 0 && newY < n) {
                if (!visited[newX][newY] && grid[newX][newY] <= threshold) {
